...
import {
  ...
  UserCheck, MessageSquare, Star, BarChart2, Bookmark
} from "lucide-react";
...
export default function EmployerDashboard() {
  ...
  const updateAppMutation = useMutation({
    /** @param {{ id: string, status: string }} variables */
    mutationFn: (variables) => base44.entities.Application.update(variables.id, { status: variables.status }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["employerApplications"] }),
  });

  const toggleShortlistMutation = useMutation({
    /** @param {{ id: string, is_shortlisted: boolean }} variables */
    mutationFn: (variables) => base44.entities.Application.update(variables.id, { is_shortlisted: variables.is_shortlisted }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["employerApplications"] }),
  });
